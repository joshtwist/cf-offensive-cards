...
export default class Cards {
  ...
  getBlackCardById(id) {
    matches = this.blackCards.filter(c => c.id === id);
    return matches[0];
  }

  getWhiteCardById(id) {
    matches = this.whiteCards.filter(c => c.id === id);
    return matches[0];
  }

  addWhiteCard(text) {
    this.whiteCards.push({
      id: this.whiteCards.length,
      text: text,
    });
  }

  addBlackCard(text) {
    this.blackCards.push({
      id: this.blackCards.length,
      text: text,
    });
  }
  ...
  getIds() {
    const bids = [];
    const wids = [];
    this.whiteCards.forEach(w => wids.push(w.id));
    this.blackCards.forEach(b => bids.push(b.id));
    return { blackCards: bids, whiteCards: wids };
  }
}
...
const blackCardsText = [
  ...
  'In the Disney Channel Original Movie, Hannah Montana struggles with _ for the first time.',
  ...
  'What is Donald J. Trump thinking about right now? _.',
  'White people like _.',
  ...
  'In M. Night Shyamalan’s new movie, Bruce Willis discovers that _had really been _ all along.',
  ...
  'Make a haiku.  _ _ _',
  'What made my first kiss so awkward? _.',
  ...
  'What’s that smell? _.',
  'After eight years in the White House, how is Obama finally letting loose? _.',
...
const whiteCardsText = [
  ...
  'Yo mamma',
  'Carole Baskin',
  'The Governer of New York having nipple piercings',
];
